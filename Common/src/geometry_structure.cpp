...
void CPhysicalGeometry::ComputeWall_Distance(CConfig *config) {
	...
	double *coord, dist2, dist, Vol, DES_Length, C_DES;
	...
	for (iPoint = 0; iPoint < GetnPoint(); iPoint++) {
		...
		/* DES lenght scale calculation and implementation*/

		if (config->GetDES()){
		    C_DES = 0.61;
		    Vol = node[iPoint]->GetVolume();
            DES_Length = pow(Vol,1.0/3.0);
            node[iPoint]->SetWall_Distance(min(C_DES*DES_Length,sqrt(dist)));
            //cout << DES_Length << endl;
		}else{
