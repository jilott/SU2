...
void COutput::MergeSolution(CConfig *config, CGeometry *geometry, CSolver **solver, unsigned short val_iZone) {
	...
	unsigned short iVar_GridVel = 0, iVar_PressMach = 0, iVar_JouleHeat = 0, iVar_Density = 0, iVar_TempLam = 0,
    ...
    double *Aux_Press, *Aux_Frict, *Aux_Heat, *Aux_yPlus, *Aux_Sens, *Aux_JouleHeat;
        ...
        if (geometry->node[iPoint]->GetDomain()) {
            ...
            switch (Kind_Solver) {
                case EULER: case FREE_SURFACE_EULER:
                    ...
                    if(jouleheating){
                        Data[jVar][jPoint] = solver[FLOW_SOL]->node[iPoint]->GetJouleHeat(); jVar++;  
                    }
                    break;
	...
	if ((Kind_Solver == EULER) || (Kind_Solver == NAVIER_STOKES) || (Kind_Solver == RANS) ||
        ...
        if (jouleheating) {
            ...
            for (iPoint = 0; iPoint < geometry->GetnPoint(); iPoint++) {
                /*--- Check for halos & write only if requested ---*/   
                if (geometry->node[iPoint]->GetDomain() || Wrt_Halo) {
                    /*--- Load buffers with the Joule heating ---*/
                    Buffer_Send_Var[jPoint] = solver[FLOW_SOL]->node[iPoint]->GetJouleHeat();             
                    jPoint++;
                }
            }
