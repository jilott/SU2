...
CSource_JouleHeating::CSource_JouleHeating(unsigned short val_nDim, unsigned short val_nVar,
    ...
    current = 1000.0;//config->GetCurrent();
    ...
    rmax = 0.2;
    dtheta = 2.0*PI_NUMBER/(nPt_theta);
    dr     = rmax/(nPt_r - 1.0);
...
void CSource_JouleHeating::ComputeResidual(double *val_residual, double **val_Jacobian_i, CConfig *config) {
    ...
    val_residual[nVar-1] = JouleHeat*Volume;
}
...
void CSource_JouleHeating::SetElec_Cond() {
    ...
    x1 = log(Patm); x2 = x1*x1; x3 = x2*x1; x4 = x3*x1; 
	...
	sigma = coeff_a[0] - coeff_c[0]*exp(-pow(log(Temperature)/coeff_d[0],w));
	...
	for (int i = 1; i < 8; ++i) {
		...
		sigma = sigma + coeff_a[i] * exp(q)/(exp(q) + exp(-q));
	}
